...
def test_chat_completion_nonstreaming_metrics(
    ...
    assert (
        "llama_stack.inference.requests_total" in metrics
    ), "requests_total metric not found"
    assert (
        "llama_stack.inference.request_duration_seconds" in metrics
    ), "request_duration metric not found"

    # Verify token-level metrics exist (if usage data is available)
    if response.usage and response.usage.total_tokens:
        assert (
            "llama_stack.inference.tokens_per_second" in metrics
        ), "tokens_per_second metric not found"
        assert (
            "llama_stack.inference.inference_duration_seconds" in metrics
        ), "inference_duration metric not found"

        # Verify tokens_per_second is reasonable (should be > 0)
        tps_metric = metrics["llama_stack.inference.tokens_per_second"]
        assert tps_metric.value > 0, "tokens_per_second should be positive"
...
def test_chat_completion_streaming_metrics(
    ...
    assert (
        "llama_stack.inference.time_to_first_token_seconds" in metrics
    ), "TTFT metric not found for streaming request"
...
def test_completion_nonstreaming_metrics(
    ...
    requests_metric = metrics["llama_stack.inference.requests_total"]
    assert (
        requests_metric.attributes.get("endpoint_type") == "completion"
    ), "endpoint_type should be 'completion'"
    assert requests_metric.attributes.get("model") == text_model_id
...
def test_error_metrics_recorded(mock_otlp_collector, llama_stack_client):
    ...
    assert (
        "llama_stack.inference.requests_total" in metrics
    ), "Should record request count even on error"
    assert (
        "llama_stack.inference.request_duration_seconds" in metrics
    ), "Should record duration even on error"

    # Verify status is error
    requests_metric = metrics["llama_stack.inference.requests_total"]
    assert (
        requests_metric.attributes.get("status") == "error"
    ), "Status should be 'error' for failed requests"


def test_concurrent_requests_metric_exists(
    mock_otlp_collector, llama_stack_client, text_model_id
):
...
def test_metric_attributes_are_consistent(
    mock_otlp_collector, llama_stack_client, text_model_id
):
    ...
    # Get metrics
    metrics = mock_otlp_collector.get_metrics(
        expect_model_id=text_model_id, timeout=10.0
    )
    ...
    assert (
        text_model_id in model_ids
    ), f"Expected model {text_model_id} in metrics, got {model_ids}"
    assert (
        "chat_completion" in endpoint_types
    ), f"Expected chat_completion endpoint_type, got {endpoint_types}"
    assert False in stream_values, f"Expected stream=False, got {stream_values}"


def test_multiple_requests_increment_metrics(
    mock_otlp_collector, llama_stack_client, text_model_id
):
    ...
    requests_metric = metrics["llama_stack.inference.requests_total"]
    assert (
        requests_metric.value >= num_requests
...
def test_tokens_per_second_calculation(
    ...
    """Verify tokens_per_second is calculated correctly when usage data is available."""
    mock_otlp_collector.clear()

    # Make a request
    response = llama_stack_client.chat.completions.create(
        model=text_model_id,
        messages=[{"role": "user", "content": "Test token calculation"}],
        stream=False,
    )

    assert response is not None

    # Only proceed if usage data is available
    if not response.usage or not response.usage.total_tokens:
        pytest.skip("Usage data not available for this provider")
    ...
    # Verify tokens_per_second exists and is reasonable
    assert "llama_stack.inference.tokens_per_second" in metrics
    tps_metric = metrics["llama_stack.inference.tokens_per_second"]

    # TPS should be positive
    assert tps_metric.value > 0, "Tokens per second should be positive"

    # TPS should be reasonable (< 10000 tokens/sec for most models)
    # This is a sanity check - if TPS is absurdly high, something is wrong
    assert (
        tps_metric.value < 100000
    ), f"Tokens per second seems unreasonably high: {tps_metric.value}"
