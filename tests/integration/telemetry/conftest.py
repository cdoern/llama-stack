    ...
    if stack_mode == "server":
        ...
        for attempt in range(max_attempts):
            try:
                conn = http.client.HTTPConnection(
                    "127.0.0.1", collector_port, timeout=1
                )
                conn.request("GET", "/")
                conn.getresponse()
                conn.close()
        ...
        class ExistingCollectorWrapper:
            ...
            def get_spans(self, **kwargs):
                ...
                try:
                    conn = http.client.HTTPConnection(
                        "127.0.0.1", collector_port, timeout=5
                    )
                    ...
                    try:
                        ...
                        if response.status == 200:
                            ...
                            for m in data.get("metrics", []):
                                ...
                                if (
                                    m["name"] not in metrics_dict
                                    or metric.value > metrics_dict[m["name"]].value
                                ):
                            ...
                            if (
                                ...
                                or len(metrics_dict) >= expected_count
                            ):
                                return metrics_dict
